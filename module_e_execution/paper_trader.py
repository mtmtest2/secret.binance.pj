...
class PaperTrader:
        ...
        async with self._lock:
            ...
            for symbol, position in list(self._positions.items()):
                ...
                if position.ladder is not None:
                    # The same state machine the backtester drives, fed ticks
                    # instead of bars.  Nothing about the ladder is
                    # re-implemented here.
    ...
    async def _close(
        ...
        if reason is CloseReason.LIQUIDATION:
            # Cannot lose more than the isolated margin committed - and profit
            # already banked by earlier ladder legs is not clawed back.
            net = max(net, -position.margin - position.realized_pnl)

        # May already carry closed ladder legs, so add rather than replace.
