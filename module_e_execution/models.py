...
class Position:
    ...
    @property
    def size(self) -> float:
        """Contracts the position was *opened* with.

        ``quantity`` is the size still open and shrinks as take-profit ladder
        legs fill, so every backward-looking quantity - notional, excursions,
        the persisted trade row - must read this instead, or a scaled-out trade
        is recorded at a fraction of the size it actually traded.
        """
        return self.initial_quantity if self.initial_quantity > 0.0 else self.quantity
    ...
    def max_favorable_excursion(self) -> float:
        ...
        if self.max_favorable_price == 0.0:
            return 0.0
        return (self.max_favorable_price - self.entry_price) * self.size * self.direction
    ...
    def max_adverse_excursion(self) -> float:
        ...
        if self.max_adverse_price == 0.0:
            return 0.0
        return (self.max_adverse_price - self.entry_price) * self.size * self.direction
