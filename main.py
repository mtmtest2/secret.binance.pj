...
Usage::
    ...
    python main.py run                      # scheduler + web panel (default)
    python main.py bootstrap                # backfill historical candles
...
class TradingSystem:
    ...
    async def startup(self) -> None:
        ...
        _LOGGER.info("Mode=%s | symbols=%d", self.trading_mode, len(self.settings.data.symbols))
        _LOGGER.info("=" * 70)
        ...
        if not all(loaded.values()):
            _LOGGER.warning(
                "Model artifacts missing (%s). Heuristic fallbacks are active and are "
                "BLOCKED from live trading - run `python main.py train` first.",
    ...
    async def _run_cycle(self, cycle_id: str) -> dict[str, Any]:
        ...
        # --- A: ingestion + QC -------------------------------------------
        bundles: dict[str, MarketDataBundle] = await self.pipeline.run_cycle()
    ...
    async def status_snapshot(self) -> dict[str, Any]:
        ...
        positions: list[dict[str, Any]] = [
            position.to_view(marks.get(symbol))
        ...
        return {
            ...
            "health": {
                **{f"model:{name}": ready for name, ready in self.ml.loaded_heads.items()},
                ...
                "symbols": str(len(self.settings.data.symbols)),
    ...
    async def set_trading_enabled(self, enabled: bool) -> dict[str, Any]:
        """Pause or resume signal generation (open positions keep being managed)."""
        self.trading_enabled = enabled
        _LOGGER.warning("Trading %s via the control API", "RESUMED" if enabled else "PAUSED")
        await self.audit.log_system_event(
            "control",
            "BLOCKED" if not enabled else "NO_TRADE",
            f"trading {'resumed' if enabled else 'paused'} by the operator",
            rule="OPERATOR_TOGGLE",
        )
    ...
    async def set_trading_mode(self, mode: str) -> dict[str, Any]:
        ...
        The outgoing engine is flattened first: a position opened by the paper
        engine has no counterpart on the exchange (and vice versa), so carrying
        one across the switch would leave it unmanaged forever.
        """
        if mode == self.trading_mode:
            return {"trading_mode": self.trading_mode, "changed": False}

        _LOGGER.warning("Switching execution mode: %s -> %s", self.trading_mode, mode)
        ...
        await self.audit.log_system_event(
            "control", "NO_TRADE", f"execution mode switched to {mode}", rule="OPERATOR_MODE_SWITCH"
        )
        return {"trading_mode": self.trading_mode, "changed": True}
    ...
    async def reset_risk_guard(self, operator: str) -> dict[str, Any]:
        """Clear a RED latch and re-enable signal generation."""
        ...
        return {"risk_guard_state": state.value, "trading_enabled": True}
    ...
    async def run_forever(self) -> None:
        """Run the scheduler and the web server side by side until shutdown."""
        await self.startup()
        self._schedule()
    ...
    async def command_bootstrap(self) -> None:
        ...
        await self.database.initialize()
        await self.fetcher.load_markets()
        written: dict[str, int] = await self.pipeline.bootstrap_history()
    ...
    async def command_train(self, max_candles: int | None = None) -> None:
        ...
        dataset: ProcessedDataset = await self.processor.build_training_dataset(
            max_candles_per_symbol=max_candles
        )
        if dataset.is_empty:
    ...
    async def command_backtest(
        ...
        report: BacktestReport = await backtester.run(
            max_candles=max_candles, initial_equity=equity
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="run",
        choices=["run", "bootstrap", "train", "backtest", "cycle"],
        help="run: scheduler + panel | bootstrap: backfill | train: fit models | "
        "backtest: replay history | cycle: one cycle then exit",
    )
