...
class DatabaseHandler:
    ...
    async def upsert_agg_trade_flow(self, buckets: Sequence[AggTradeFlow]) -> int:
        ...
        Batched exactly like :meth:`upsert_candles`: a multi-month aggTrades
        backfill produces one row per 5-minute bucket per symbol, which is the
        same order of magnitude as the candle table itself and well past
        SQLite's bound-parameter ceiling for a single statement.

        Re-ingesting a window is safe and is the intended repair path - a
        bucket written from a partially fetched page is overwritten by the
        complete one on the next pass.
        """
        if not buckets:
            return 0
        ...
        try:
            async with self._write_lock, self._factory()() as session:
                async with session.begin():
                    for batch in chunked(payload, batch_rows):
                        statement = sqlite_insert(AggTradeFlowRow).values(batch)
                        statement = statement.on_conflict_do_update(
                            index_elements=[
            ...
            raise DatabaseError(
                "agg trade flow upsert failed", rows=len(payload), reason=str(error)
            ) from error
        return len(payload)
    ...
    async def earliest_agg_trade_flow_timestamp(self, symbol: str) -> int | None:
        """Oldest stored order-flow bucket for ``symbol``, or ``None``.

        The backfill walks *backwards* from here, mirroring the futures-metrics
        resume logic, so an interrupted deep backfill continues where it
        stopped instead of re-fetching the recent tail it already has.
        """
        query: Select[Any] = select(func.min(AggTradeFlowRow.timestamp)).where(
            AggTradeFlowRow.symbol == symbol,
            AggTradeFlowRow.timeframe == self._settings.data.timeframe,
        )
        async with self._factory()() as session:
    ...
    async def candle_range(self, symbol: str) -> tuple[int | None, int | None]:
        ...
        Used to clamp the order-flow backfill to the window candles actually
        exist for: a flow bucket outside that range has no bar to attach to.
        """
        query: Select[Any] = select(
