...
# ---------------------------------------------------------------------------
def test_ladder_section_reports_configuration_and_outcome() -> None:
    """The /ml-report page must show TP/SL behaviour, not just bury it."""
    from module_f_panel import diagnostics

    section = diagnostics._take_profit_ladder(
        Settings(),
        {
            "status": "AVAILABLE",
            "metrics": {
                "pct_reached_tp1": 0.74,
                "pct_reached_tp2": 0.51,
                "pct_reached_tp3": 0.29,
                "pct_stopped_initial": 0.26,
                "pct_stopped_breakeven": 0.23,
                "pct_stopped_tp1_locked": 0.21,
                "average_r": 0.93,
                "average_trade_return": 0.0017,
                "long_trades": 135.0,
                "short_trades": 98.0,
            },
        },
    ...
    assert section["close_fractions"] == [0.30, 0.30, 0.40]
    assert section["pct_reached_tp1"] == pytest.approx(0.74)
    assert section["pct_stopped_breakeven"] == pytest.approx(0.23)
    assert section["long"]["long_trades"] == pytest.approx(135.0)
    assert section["short"]["short_trades"] == pytest.approx(98.0)


def test_ladder_section_is_explicit_when_no_backtest_ran() -> None:
    """Absence of data is reported as such, never as a zero-filled ladder."""
    from module_f_panel import diagnostics

    section = diagnostics._take_profit_ladder(Settings(), None)
    assert section["status"] == "NOT_AVAILABLE"
    assert "reason" in section
    # Configuration is still reported, so a disabled ladder is distinguishable
    # from one that simply had no trades to act on.
    assert "level_fractions" in section
